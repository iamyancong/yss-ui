...
发版时 MCP 必须在组件包版本写入并发布后重建索引；凡发布 `@yss-ui/components`，发布流水线强制跟发派生包 `@yss-ui/mcp` 以同步最新索引与消费契约；发布后校验门禁若发现组件已发布但 MCP 缺失将直接拒绝放行，GitHub Actions 亦会从 registry tarball 复核 `componentsVersion` 与本批组件目标版本一致，防止工作区索引和最终发布包漂移。
